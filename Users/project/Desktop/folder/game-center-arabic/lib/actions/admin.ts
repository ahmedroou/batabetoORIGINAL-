...
export const countQuestions = withAdminAuth(async (adminId: string, criteria: { game: 'prison' | 'word_war' , all?: boolean, duplicates?: 'word_war_duplicates' }) => {
    if (!criteria.all && !criteria.duplicates) {
...
export const deleteQuestions = withAdminAuth(async (adminId: string, criteria: { game: 'prison' | 'word_war', all?: boolean }) => {
    if (!criteria.all) {
