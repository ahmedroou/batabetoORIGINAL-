...
const HowToPlayDialog = () => (
    <Dialog>
        ...
        <DialogContent className="max-w-md">
            <DialogHeader>
                <DialogTitle>كيفية لعب التاجر الذكي</DialogTitle>
                <DialogDescription>
            ...
            <div className="prose prose-sm dark:prose-invert max-h-96 overflow-y-auto pr-4">
                ...
                <ul>
                    ...
                    <li><strong>ماذا يحدث عند التوقف؟</strong>
                        <ul>
                            <li><strong>عقار غير مملوك:</strong> يمكنك شراؤه. لإتمام الشراء، يجب عليك الإجابة على سؤال بشكل صحيح. إذا أجبت خطأ، تسترجع ربع المبلغ. يمكنك أيضًا تخطي الشراء.</li>
                            ...
                            <li><strong>بطاقة غرامة:</strong> سيتم تطبيق التأثير المالي عليك بشكل فوري.</li>
                        </ul>
                    </li>
                </ul>

                <h4>4. الإفلاس</h4>
                <p>إذا انخفض رصيدك إلى ما دون الصفر، تعتبر مفلساً وتخرج من اللعبة. تعود جميع ممتلكاتك إلى البنك وتصبح متاحة للشراء.</p>
                
                <h4>5. نهاية اللعبة</h4>
                <p>تنتهي اللعبة إما بخروج جميع اللاعبين باستثناء لاعب واحد، أو بانتهاء عدد الجولات المحددة. يتم بعد ذلك حساب الثروة الإجمالية لكل لاعب، واللاعب الأكثر ثراءً هو الفائز.</p>
            </div>
        </DialogContent>
    </Dialog>
);
...
export function LobbyPhase({ game, self }: LobbyPhaseProps) {
    ...
    const [settings, setSettings] = useState(game.smartMerchantState?.settings || { rounds: 15 });
    ...
    const handleSaveSettings = async () => {
        ...
        setIsSubmitting(true);
        try {
            await smartMerchantActions.updateGameSettings(game.id, self.id, settings);
        ...
        <>
            <Card className="w-full max-w-md animate-bounce-in">
                <CardHeader className="text-center">
                    <CardTitle className="text-2xl">التاجر الذكي</CardTitle>
