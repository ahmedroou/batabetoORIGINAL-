                           ...
                           <svg className="snake-ladder-svg">
                            ...
                            {players.map((p) => {
                                ...
                                return (
                                    ...
                                    >
                                        <PlayerAvatar avatarId={p.avatarId} className="w-full h-full border-2 border-white rounded-full shadow-lg" temporaryTitle={p.temporaryTitle} />
                                    </motion.div>
                                );
                            })}
                         </div>
                     ...
                     <div className="absolute bottom-0 left-0 right-0 h-16 flex justify-center items-center gap-2 p-2 bg-slate-900/50 rounded-b-lg">
                        {players.filter(p => p.position === 0).map(p => (
                             <motion.div key={`start-avatar-${p.id}`} layoutId={`player-avatar-${p.id}`}>
                                <PlayerAvatar avatarId={p.avatarId} className="w-10 h-10 border-2 border-primary rounded-full" temporaryTitle={p.temporaryTitle} />
                            </motion.div>
                        ))}
                    </div>
                </div>
