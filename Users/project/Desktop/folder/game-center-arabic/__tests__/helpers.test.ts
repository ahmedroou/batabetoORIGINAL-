...
describe('safeCompareStrings', () => {
    ...
    // Test for Arabic punctuation (question mark)
    it('should ignore Arabic punctuation', () => {
        expect(safeCompareStrings('ما هي عاصمة مصر؟', 'ما هي عاصمة مصر')).toBe(1.0);
    });

    // Test for Arabic diacritics (Tanween)
    it('should handle Arabic diacritics', () => {
        expect(safeCompareStrings('كتابٌ', 'كتاب')).toBe(1.0);
        expect(safeCompareStrings('كتاباً', 'كتاب')).toBe(1.0);
        expect(safeCompareStrings('كتابٍ', 'كتاب')).toBe(1.0);
    });
});
