...
import { updateGameSettings, startGame, submitHint, revealCard, endTurn, handleTimeout, toggleSuspicion, proceedToFinalResults, setGuide, randomizeTeams, selectTeam as selectWordWarTeam } from '@/lib/actions/word-war';
...
export function WordWarGame({ game, self }: WordWarGameProps) {
    ...
    if (game.gameState === 'board_reveal' || game.gameState === 'final_results') {
        return (
             <Card className="w-full max-w-4xl animate-bounce-in bg-background">
                <CardHeader className="text-center">
                    <Crown className="w-24 h-24 mx-auto text-yellow-400" />
                    <CardTitle className="text-4xl">انتهت اللعبة!</CardTitle>
                    <CardDescription className={cn("text-2xl font-bold", game.gameResult?.winner === 'red' ? 'text-red-500' : 'text-blue-500')}>
                        {game.gameResult?.message}
                    </CardDescription>
                </CardHeader>
                <CardContent className="grid grid-cols-5 md:grid-cols-8 gap-2 p-2 max-w-7xl mx-auto">
                     {wwState.cards.map((card, index) => (
                        <motion.div
                            key={index}
                            initial={{ opacity: 0, y: -20 }}
