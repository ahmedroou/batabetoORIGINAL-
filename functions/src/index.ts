...
/**
 * A scheduled function that runs every Thursday at 10:00 AM UTC to recalculate and update the "Game Kings".
 * This function is robust and crucial for keeping the leaderboards up-to-date automatically.
 */
export const updateGameKings = onSchedule("every thursday 10:00", async (event) => {
    ...
    for (const gameType of gameTypes) {
      ...
      if (!snapshot.empty) {
        ...
        const newKingData: GameKing = {
          ...
          updatedAt: admin.firestore.FieldValue.serverTimestamp() as any, // Use server timestamp for consistency
        };
        
        batch.set(gameKingRef, newKingData, { merge: true });
        updatedCount++;
        logger.info(`New king for ${gameType}: ${kingData.name} with ${newKingData.winCount} wins.`);
      }
    }
