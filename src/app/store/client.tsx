...
export default function StoreClient() {
  ...
  const fetchPageData = useCallback(async () => {
    ...
    if (
      punishmentPricesResult.success &&
      punishmentPricesResult.prices
    ) {
  ...
  const visibleAvatarIds = useMemo(() => {
    const all =
      storeTab === "regular" ? AVATAR_IDS : PUNISHMENT_AVATAR_IDS;
    const unlocked =
      storeTab === "regular"
  ...
  return (
    <main
      dir="rtl"
      className="min-h-screen bg-[radial-gradient(50%_50%_at_50%_10%,rgba(147,51,234,0.12),transparent_60%),linear-gradient(to_bottom_right,rgba(2,6,23,0.9),rgba(17,24,39,0.9))] text-foreground"
    >
      <div className="container relative z-10 mx-auto px-4 py-8">
        <header className="relative mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            تصفح واشترِ شخصيات جديدة لتتميز بها داخل اللعبة.
          </p>
          <Button
            variant="ghost"
            size="icon"
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="ابحث..."
                    className="w-[220px] pe-8 bg-background/20"
                  />
                </div>
                <Select
                  value={filter}
            ...
            {isLoadingData ? (
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                {Array.from({ length: 12 }).map((_, i) => (
                  <Skeleton
                    key={i}
                    className="aspect-square w-full rounded-xl bg-slate-700/50"
                  />
                ))}
              </div>
            ) : (
              <ScrollArea className="h-[65vh] rounded-md border border-purple-500/20 bg-black/20 p-3">
                {visibleAvatarIds.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-8">
                    ...
                    <p className="text-sm">
                      جرّب تغيير كلمات البحث أو التصفية.
                    </p>
                  </div>
                )}
              </ScrollArea>
      ...
      <AlertDialog
        open={!!itemToPurchase}
        onOpenChange={() => setItemToPurchase(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>تأكيد الشراء</AlertDialogTitle>
            <AlertDialogDescription>
              هل أنت متأكد من رغبتك في شراء هذه الشخصية مقابل{" "}
              {itemToPurchase?.price}{" "}
              {prettyCurrency(itemToPurchase?.currency || "coins")}؟
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
