...
function CityGridCell({ cell, onDrop, onSpecialClick }: CityGridCellProps) {
  ...
  return (
    <motion.div
      ...
      key={`${cell.x}-${cell.y}`}
      className={cn(
        'w-20 h-20 border border-green-800/30 rounded-sm flex items-center justify-center transition-colors relative group shadow-inner-light',
...
export function CityGrid({ city, onPlaceItem, onSpecialClick }: CityGridProps) {
  return (
    <div className="p-8 bg-black/20 rounded-lg shadow-inner-dark inline-block select-none pointer-events-none">
      <div
