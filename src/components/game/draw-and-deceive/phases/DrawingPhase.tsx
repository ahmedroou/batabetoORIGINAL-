...
import { 
  Undo2, Redo2, Eraser, Pencil, Highlighter, MousePointer, Type, Droplet, Image as ImageIcon, Download,
  Maximize2, Minimize2, Square, Circle, Minus, Grid, Trash2, Hand
} from 'lucide-react';
...
export function DrawingCanvas({
  ...
  const pushHistory = useCallback((custom?: string) => {
    const dataUrl = custom ?? canvasRef.current!.toDataURL('image/png', 0.95);
    setHistory(prev => {
    ...
    setHistoryIndex(idx => {
      const prevLen = historyIndex >= 0 ? history.slice(0, historyIndex + 1).length : 0;
      const newLen = Math.min(maxHistory, (historyIndex >= 0 ? prevLen : 0) + 1);
      return newLen - 1;
    });
    if (onDrawEnd) onDrawEnd(dataUrl);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [onDrawEnd, maxHistory, historyIndex, history]);
  ...
  const undo = () => {
    ...
    img.onload = () => {
      ...
      ctx.clearRect(0, 0, size.w, size.h);
      ctx.drawImage(img, 0, 0, size.w, size.h);
      if (onDrawEnd) onDrawEnd(history[newIndex]);
    };
  };
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    if (tool === 'eyedropper') {
      ...
      // أخذ اللون من الصورة مع احتساب pan/zoom تم بالتبديل قبل
      // أبسط طريقة: نقرأ من canvas الظاهر بعد تحييد التحويلات
      const data = ctx.getImageData(0, 0, size.w, size.h).data;
      const idx = ((Math.floor(pt.y) * size.w) + Math.floor(pt.x)) * 4;
  ...
  const clearAll = () => {
    ...
    setHistory([]);
    setHistoryIndex(-1);
    if (onDrawEnd) onDrawEnd(canvasRef.current!.toDataURL('image/png', 0.95));
  };
