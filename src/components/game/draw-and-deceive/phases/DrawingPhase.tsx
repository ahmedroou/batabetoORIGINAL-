...
import {
  Loader2, Send, Timer, Pen, Eraser, Minus, Square, Circle, Triangle, Type, MousePointer, PaintBucket, Grid, Undo2, Redo, Trash2, Download, Copy, Upload, Brain, Eye
} from 'lucide-react';
...
const ColorSwatch = memo(({ color, ...props }: { color: string } & React.ComponentProps<'button'>) => (
  <button {...props}>
...
const BrushGroup = memo(({ tool, setTool }: { tool: any; setTool: (t: any) => void }) => (
  <Popover>
...
BrushGroup.displayName = 'BrushGroup';

const ShapesGroup = memo(({ tool, setTool }: { tool: any; setTool: (t: any) => void }) => {
...
ShapesGroup.displayName = 'ShapesGroup';

const ColorGroup = memo(({ color, setColor }: { color: string; setColor: (c: string) => void }) => {
...
const WaitingView = memo(({ title, description }: { title: string; description: string }) => (
    <Card className="w-full max-w-lg text-center">
      <CardHeader>
