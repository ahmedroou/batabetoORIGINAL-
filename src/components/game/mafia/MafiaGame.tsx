...
const RoleRevealPhase = ({ game, self, isHost, isSubmitting, setIsSubmitting }: { game: Game; self: Player; isHost: boolean; isSubmitting: boolean; setIsSubmitting: (isSubmitting: boolean) => void; }) => {
        ...
        return (
             <>
                <CardHeader className="text-center">
                 ...
                 {isHost && (<CardFooter><Button onClick={() => mafiaActions.hostProgressNextPhase(game.id, self.id)} className="w-full">الانتقال إلى الليل</Button></CardFooter>)}
            </>
        );
     };
...
const FinalResultsPhase = ({ game, handleLeaveGame }: { game: Game; handleLeaveGame: () => void; }) => {
    ...
    return (
        <Card className="w-full max-w-md text-center">
            <CardHeader>
                <Users className="w-24 h-24 mx-auto text-yellow-400" />
            ...
            <CardContent><p className="text-lg text-muted-foreground">{message}</p></CardContent>
...
export function MafiaGame({ game, self }: MafiaGameProps) {
  ...
  const renderContent = () => {
    switch (game.gameState) {
      ...
      case 'role_reveal':
        return <RoleRevealPhase game={game} self={self} isHost={isHost} isSubmitting={isSubmitting} setIsSubmitting={setIsSubmitting} />;
      case 'night':
