 ...
 * Trap Answer — UI (cleaned & de-flickered)
 *
 * Highlights:
 * - Removes pop-in reanimation on every minor state change (fixes brief screen flash on selection).
 * - Stable keys/ids for answer options; no re-mounts while toggling.
 * - Top-level hooks only, trimmed dependencies to reduce unnecessary re-renders.
 * - Safe guards for undefined state; accessible live regions.
 * - Keeps the “merge-by-text” logic intact (answers are compared by string).
 */
...
export function TrapAnswerGame({ game, self }: TrapAnswerGameProps) {
  ...
  // ---------------- Timer wrapper (de-flickered) ----------------
  // NOTE: removed 'animate-pop-in' to stop flashing on any state change.
  ...
  }, [game.id, self.id, trapAnswer, toast, game.trapAnswerState?.playerAnswers, setLoading]);
...
function TrapAnswerLobby({ game, self }: { game: Game; self: Player }) {
  ...
  // Sync settings if updated by server
  useEffect(() => {
