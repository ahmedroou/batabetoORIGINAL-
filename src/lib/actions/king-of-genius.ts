...
// Generates puzzle for the current index and prepares the game state for the active challenge.
async function prepareNextChallenge(gameId: string, transaction: Transaction) {
    const gameRef = doc(db, 'games', gameId.toUpperCase());
    const gameDoc = await transaction.get(gameRef);
    ...
    // Generate the puzzle and update the game state for the active phase.
...
export async function nextKingOfGenius(gameId: string, hostId: string) {
    ...
    await runTransaction(db, async (transaction) => {
        ...
        if (game.gameState !== 'challenge_results') return;
...
export async function handleTimeout(gameId: string, hostId: string) {
    let finalGameData: Game | null = null;
    let needsPuzzleGeneration = false;
    ...
    await runTransaction(db, async (transaction) => {
        ...
        switch(game.gameState) {
            case 'challenge_intro':
                // The intro is over, time to prepare the actual challenge.
                // We will set this flag and run the heavy generation logic after the transaction.
                needsPuzzleGeneration = true;
                break;
            case 'challenge_active':
                ...
                // Read the updated doc to get all results before calculating scores
                const updatedGameDoc = await transaction.get(gameRef);
                const updatedGame = updatedGameDoc.data() as Game;

                const results = updatedGame.challengeState?.results || [];
                const sortedCorrectResults = results.filter(r => r.isCorrect).sort((a, b) => {
            ...
            case 'challenge_results':
                // This will trigger the next round or end the game
    ...
    if (needsPuzzleGeneration) {
        await runTransaction(db, async (tx) => await prepareNextChallenge(gameId, tx));
    }
}
