...
const mathPuzzlePrompt = ai.definePrompt({
  ...
  prompt: `أنت مساعد خبير ومبدع في تصميم الألعاب. مهمتك هي إنشاء لغز فريد وعشوائي لتحدي "الحساب السريع".

قم بتوليد 5 مسائل حسابية جديدة ومختلفة في كل مرة يتم استدعاؤك فيها.
يجب أن تزداد صعوبة المسائل تدريجيًا من حيث عدد العمليات، وليس حجم الأرقام.
- **الأرقام:** استخدم أرقامًا مكونة من رقمين على الأكثر (e.g., between 1 and 99).
- **العمليات:** يجب أن تتضمن المسائل عمليات الضرب والجمع والطرح.
- **الصعوبة:**
    - المسائل الأولى: عمليتان حسابيتان (e.g., 12 * 4 - 5).
    - المسائل الأخيرة: ثلاث عمليات حسابية واستخدام الأقواس (e.g., 7 * (11 + 3) - 15).
- **النتيجة:** تجنب القسمة والأرقام السالبة في النتيجة النهائية.
...
مثال للمخرجات:
{
  "problems": [
    ...
    { "problem": "80 - 15 * 3 + 2", "answer": 37 },
    { "problem": "9 * (5 + 6) - 11", "answer": 88 }
  ]
}
